# Luma Estate OS - Copy Bank

## Frase Principal (Core Message)
> “No construyo páginas web. Construyo infraestructura comercial inmobiliaria para captar, medir y cerrar prospectos de alto valor.”

## Hero Copy
**Headline:** Propiedades Premium. Experiencias de venta amateur.
**Subheadline:** Si su equipo no sabe qué campaña generó qué venta, está perdiendo dinero. Luma Estate OS es el puente definitivo entre su marketing y sus comisiones.

## CTAs (Call to Actions) - Jerarquía Consistente
- **Principal (Landing / Botones Core):** Solicitar Auditoría Comercial
- **Secundario (Educación):** Ver Caso de Segmentación
- **Submit (Formulario):** Solicitar Evaluación Ejecutiva

## Pitch Corto
"Luma Estate OS es una infraestructura comercial diseñada para propiedades de alto nivel. Conectamos una presentación visual impecable con un backend de datos preciso. Es la diferencia entre recibir un email genérico y tener visibilidad total de su ciclo de ventas."

## Pitch Ejecutivo (C-Level)
"Señor prospecto, actualmente hay una desconexión en el mercado inmobiliario de lujo. Se invierten millones en construir propiedades espectaculares, pero el cliente termina en un formulario genérico. Yo instalo Luma Estate OS para conectar una presentación visual que enamora con un pipeline de ventas implacable que le dice a su equipo exactamente de dónde vino el lead y cuándo cerrarlo."

## Copy para Presentar el Demo (Vista del Río)
"El caso de la Segmentación Inmobiliaria: No vendemos el edificio, vendemos la oportunidad adaptada. A través de Luma Estate OS, creamos 3 terminales de captación distintas para la misma propiedad (Inversor Extranjero Ley 171-07, Airbnb, Corporativo). Las tres alimentan el mismo CRM, pero el mensaje está adaptado a quien tiene el dinero."

## Objeciones y Respuestas
- **"Ya tenemos un CRM (Salesforce/HubSpot)"**: Luma Estate OS no lo reemplaza, lo alimenta. Construimos las terminales de captación que inyectan leads pre-calificados y con contexto a su CRM actual.
- **"Es muy costoso"**: El costo de no tener atribución es mayor. Un solo lead de $500k que se enfríe paga la implementación de todo el sistema.
- **"El mercado está lento ahora"**: Exacto. Cuando hay menos compradores, necesita la infraestructura más fina para dar el mejor seguimiento.

## Copy para Ads & Outreach
- "El diseño premium atrae. La infraestructura sólida cierra."
- "Sustituya la intuición de sus agentes por visibilidad comercial absoluta."
- "No compita por precio. Compita por la experiencia de compra."
- "Capturar un lead de alto patrimonio con un formulario genérico es como servir un vino de reserva en un vaso de plástico."
- "Una propiedad de alto valor no debería depender de seguimiento improvisado."
- "Si no puede medir qué campaña genera oportunidades reales, su operación comercial está funcionando por intuición."
- "Una landing aislada captura. Un sistema conectado convierte."
- "Luma Estate OS protege los prospectos que sus campañas ya pagaron."

## Firma de Autoría Permanente (Footer)
> Luma Estate OS es una vertical comercial de Luma Premium.
> Infraestructura Comercial Inmobiliaria
> © [Año] Marcos Hilario. Arquitectura Digital de Alto Rendimiento.

*Nota de estilo: La autoría debe tener presencia visual premium con contraste alto (ej. texto blanco y nombre en negrita) para reforzar la marca personal y la percepción high-ticket.*

## Sales Kit - Promesa y Tono
**Promesa Central:** "En 21 a 30 días organizamos la operación comercial inmobiliaria con landings, formularios, CRM, pipeline, atribución, seguimiento y dashboard ejecutivo."

**Regla de Outreach:** No perseguir. Filtrar. Buscamos operaciones que ya tienen el dolor de la desorganización de leads.